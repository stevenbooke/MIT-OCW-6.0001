...
class NewsStory(object):
    ...
    def get_pubdate(self):
        return self.pubdate # datetime is immutable so I dont need to return a copy
# TODO: NewsStory
...
class PhraseTrigger(Trigger):
    ...
    def is_phrase_in(self, text):
        parsed_text = ''
        for c in text:                          # Replace punctuations in the text with spaces
            if c not in string.punctuation:
                parsed_text += c
            else:
                parsed_text += ' '
        parsed_text = parsed_text.lower().split()
        parsed_phrase = self.get_phrase().lower().split()
        flag = False
        for i, word in enumerate(parsed_text):
            if word == parsed_phrase[0] and parsed_text[i:i + len(parsed_phrase)] == parsed_phrase:
                return True
        return False
...
class BeforeTrigger(TimeTrigger):
    ...
    def evaluate(self, story):
        try:
            flag = story.get_pubdate() < self.time
        except:
